...
void fillCID(String mm)
{
  ...
  */
  ...
  //將第二個字元設為 'w'
  clintid[1] = 'w' ;
  //將 mm 轉換為字元陣列，複製到 clintid 陣列中
  mm.toCharArray(&clintid[2], mm.length() + 1) ;
  //在 clintid 陣列的最後一個位置加上換行符號 '\n'
  clintid[2 + mm.length() + 1] = '\n' ;
}



void initMQTT()   //這段程式主要是用來初始化 MQTT Broker 的連線
{
  mqttclient.setServer(MQTTServer, MQTTPort);
  // 設定要連接的 MQTT Server 的 URL 和 Port，變數 MQTTServer 存放 Server 的 URL，MQTTPort 存放 Server 的 Port

  mqttclient.setCallback(mycallback);
  // 設定回呼函式，當訂閱的主題有訊息時，會呼叫 mycallback 函式

  fillCID(MacData);  // 呼叫 fillCID 函式，產生一個隨機的 client id，並儲存在全域變數 clintid 中
  ...
  mqttclient.setServer(MQTTServer, MQTTPort);
  // 再次設定要連接的 MQTT Server 的 URL 和 Port，以防有未設定到的情況

  mqttclient.setCallback(mycallback);
  // 再次設定回呼函式

  connectMQTT();   // 呼叫 connectMQTT 函式，連接到 MQTT Server
}

void connectMQTT()  //這個函數主要是用來連接 MQTT Broker
{
  //這個函數主要是用來連接 MQTT Broker
  Serial.print("MQTT ClientID is :(") ;
  Serial.print(clintid) ;
  Serial.print(")\n") ;
  // 印出 MQTT Client 基本訊息，例如 Client ID
  ...
  Serial.println("\n MQTT connected!");
  // 連接 MQTT Broker 成功，印出訊息
}

boolean ReadModbusDeviceStatus()  //讀取Modbus Device內所有感測裝置之狀態，並送到MQTT Broker上
{
...
void mycallback(char* topic, byte* payload, unsigned int length)  //這段程式碼是用來處理 MQTT 訊息的回呼函式
{
  //這段程式碼是用來處理 MQTT 訊息的回呼函式
  
  // 這是 MQTT 的回呼函式，參數格式固定，勿更改
  // 它會在有新訊息到達時被呼叫

  String payloadString; // 宣告一個字串來存放 payload
  // 顯示訂閱內容
  Serial.print("Incoming:(") ;
  for (int i = 0; i < length; i++)
  {
    payloadString = payloadString + (char)payload[i];
    // 將 payload 轉成字串並儲存到 payloadString 中
  ...
  // 顯示收到的訊息內容
  json_doc.clear() ;    //重設空值
  deserializeJson(json_doc, payloadString);
  // 解析收到的 JSON 資料
  ...
  const char* c1 = "HIGH" ;
  const char* c2 = "LOW" ;
  const char* macadd = MacData.c_str() ;
  if (strcmp(devname, macadd) !=0)    //為 驅動裝置不同
    {
        Serial.println("MAC Address is not belong this device") ;
        return ;
    }
  // 取出 DEVICE 和 COMMAND 屬性的值
  Serial.print("OP=");
  Serial.print(op);
  Serial.print("/c1=");
  Serial.print(c1);
  ...
  Serial.print(c2); 
   Serial.print("/=====\n");
  if (strcmp(sensorname, st)==0)    //為 "RELAY"
  {
    if (strcmp(op, c1)==0)    //為 "HIGH"
    {
      Serial.println("NOW -----is  HIGH------------");
      genOpenRelay(setnum-1) ; //產生第n號繼電器 開啟之命令內容
    ...
    if (strcmp(op, c2)==0)    //為 "LOW"
    {
          Serial.println("NOW -----is  LOW------------");
      genCloseRelay(setnum-1) ;   //產生第n號繼電器 關閉之命令內容
      ShowcmdHex(cmd_Controlrelay, 8) ;  //用16進位方式顯示cmd_Controlrelay內容
      SendModbusCommand(cmd_Controlrelay, 8);  //送cmd_Controlrelay 到modbus device內
      delay(200);   //延遲0.2秒
      receivedlen = GetRS485data(&receiveddata[0]) ;  //資料長度
