...
{
  String chkstring  ;
  int len = hexstr.length() ; // Compute the length of the input string

  unsigned int i = 0 ;
  unsigned int tmp = 0 ;
  unsigned int tmp1 = 0 ;
  unsigned long tmpnum = 0 ;
  String hexcode = String("0123456789ABCDEF") ; // String containing hexadecimal digits
  for (i = 0 ; i < (len ) ; i++) // Iterate through the input string
  {
    hexstr.toUpperCase() ; // Convert each character in the input string to uppercase
    tmp = hexstr.charAt(i) ; // Get the ASCII code of the character at position i
    tmp1 = hexcode.indexOf(tmp) ; // Find the index of the character in the hexcode string
    tmpnum = tmpnum + tmp1 * POW(base, (len - i - 1) )  ; // Update the value of tmpnum based on the value of the current character
  }
  return tmpnum; // Return the resulting unsigned long integer
}
