/*
 CRC16是一種循環冗餘檢查（Cyclic Redundancy Check，CRC）的演算法，
 它通常用於檢查數據在傳輸過程中是否出現錯誤。
 CRC16通過對原始數據進行多項式除法運算，
 得到一個16位的校驗值，
 這個校驗值可以用來檢查數據在傳輸過程中是否被修改。
 當接收方接收到數據時，也可以對數據進行同樣的運算，
 然後比較接收到的校驗值和計算出的校驗值是否相等，
 如果不相等，則說明數據已經被修改，
 需要重新傳輸或採取其他措施來修正錯誤。
 在程式碼中，
